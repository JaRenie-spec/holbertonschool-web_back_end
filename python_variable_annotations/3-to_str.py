#!/usr/bin/env python3

"""
This module provides a function to convert a float to its string representation.

The function takes a floating-point number and returns its corresponding string representation.
"""

def to_str(n: float) -> str:
    """
    Converts a float to its string representation.

    This function takes a float and returns its corresponding string representation.

    Parameters:
        n (float): The float number to be converted to a string.

    Returns:
        str: The string representation of the input float.
    """
    return str(n)
